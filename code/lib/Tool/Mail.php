...
class Mail
{
     ...
     * @param  string $fileName Attached file name
     * @return void
     */
    public function pushAttached($filePath, $fileName = null)
    {
        if (file_exists($filePath)) {
            $fileName = $fileName !== null ? $fileName : basename($filePath);
            $this->_attached[] = array('path' => $filePath, 'name' => $fileName);
        }
    }
    ...
    public function send($to, $from_name = NULL, $from_ad = NULL) {
        if (count($this->_attached)) {
            return $this->sendWithFile($to, $from_name, $from_ad);
        }
    ...
    public function sendWithFile($to, $from_name = NULL, $from_ad = NULL)
    {
        if (! count($this->_attached)) {
            return $this->send($to, $from_name, $from_ad);
        }
        ...
        // 添付ファイル
        foreach($this->_attached as $attached) {
            $contentType = $this->getContentType($attached['path']);
            $fileBody = file_get_contents($attached['path']);
            $fileName = $this->beforeMimeHeaderText($attached['name']);

            $mailBody .= '--' . $boundary . "\n";
            $mailBody .= 'Content-Type: ' . $contentType . '; name=' . $fileName . "\n" .
                         'Content-Transfer-Encoding: base64' . "\n" .
                         'Content-Disposition: attachment; filename=' . $fileName . "\n";
            $mailBody .= "\n";
            $mailBody .= chunk_split(base64_encode($fileBody)) . "\n";
            $mailBody .= "\n";
        }
    ...
    private function getContentType($filePath)
    {
        ...
        if (false !== ($D = @getimagesize($filePath))) {
            list($width, $height, $type, $attr) = $D;
